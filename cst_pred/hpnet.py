...
class PrimitivesEmbeddingDGCNGn(nn.Module):
    ...
    def __init__(self, opt, emb_size=50, num_primitives=8, primitives=True, embedding=True, parameters=True, mode=0,
    ...
    def forward(self, points, normals, end_points=None, inds=None, postprocess=False):
        ...
        if postprocess:
            np.random.seed(1234)
        np.random.shuffle(l)
        random_index = torch.from_numpy(l[:self.opt.npoints])
        ...
        # print("subidx: ", subidx.shape)
        num_points = points.shape[2]
        # print("points: ", points.shape)
        x, first_layer_features = self.encoder(points)
        # print("error happen at: x, first_layer_features = self.encoder(points)")
        ...
        # print("error happen at: x = torch.cat([x, first_layer_features], 1)")
